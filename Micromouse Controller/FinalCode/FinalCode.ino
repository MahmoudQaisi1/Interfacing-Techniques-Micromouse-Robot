  ...
  if (millis() - prevTime >= interval) {
    if(flag){
      ...
      if (analogRead(IR_pin) > 512 && analogRead(LIR_pin) < 512 && analogRead(RIR_pin) < 512) // if there's a wall on left and right move forward
        MoveForwardConstant();
      else if (analogRead(LIR_pin) > 512 && distanceL > 28){
        encoderACounter = 0;
        ...
        Stop();
          encoderACounter = 0;
          while(encoderACounter < 112){
          NightyDegreesLeft();
          }
        Stop();
        flag = true;
      } else if (analogRead(RIR_pin) > 512  &&  analogRead(IR_pin) < 512){
        // encoderACounter = 0;
...
void MoveForwardSlowly() {
  ...
  analogWrite(motorA_pin_en, rightSpeed -10);
  analogWrite(motorB_pin_en, leftSpeed - 10);
}
...
if (analogRead((LIR_pin) > 512 || analogRead(RIR_pin) > 512) && (distanceL > 30 || distanceR > 30)){
